...
import com.fyjf.modules.user.web.LoginController;
...
public class SystemAuthorizingRealm extends AuthorizingRealm {
	...
	protected AuthenticationInfo doGetAuthenticationInfo(AuthenticationToken authcToken) {
		...
		// 校验用户名密码
		User user = getSystemService().getUserByAccount(token.getUsername());
		if (user != null) {
	...
	protected AuthorizationInfo doGetAuthorizationInfo(PrincipalCollection principals) {
		...
		// 获取当前已登录的用户  
		if (!Global.TRUE.equals(Global.getConfig("user.multiAccountLogin"))){//是否容许多账号登录
		...
		User user = getSystemService().getUserByAccount(principal.getAccount());
		if (user != null) {
			SimpleAuthorizationInfo info = new SimpleAuthorizationInfo();
			//这个地方用户的菜单是静态菜单  静态权限
			/*			
			List<Menu> list = getSystemService().getUserMenus(user);
			for (Menu menu : list){
...
//				info.addRole(role.getId());//(role.getEnname());
//			}
 * 
 */
			// 更新登录IP和时间
...
//		private Map<String, Object> cacheMap;
		...
		public Principal(User user) {
			this.id = user.getId();
			this.account = user.getAccount();//user.getLoginName();
			this.userName = user.getName();
			this.companyId = user.getCompany()!=null?user.getCompany().getId():"";
			this.companyName = user.getCompany()!=null?user.getCompany().getName():"";
			this.officeId = user.getOffice()!=null?user.getOffice().getId():"";
			this.officeName = user.getOffice()!=null?user.getOffice().getName():"";
